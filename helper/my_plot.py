...
# 단변량 커널 밀도 그래프 그리기
    sb.kdeplot(data=data, x=x, fill=fill, linewidth=linewidth, palette=palette, hue=hue, ax=ax)

# 신뢰구간 표기 (신뢰수준이 0이 아닌 경우에만)
    if clevel:
...
# 평균선 표시
    if meanline:
        ...
        if hue is None:
            ...
            ax.text(x=mv + 0.5, y=y_max * 0.95, s=f'Mean: {mv: 2f}', color='red', fontsize=14, ha='center')

        else:
            # hue 범주별 평균선 표기 (kdeplot이 그린 라인의 색상과 일치시킴)
            categories = list(data[hue].unique())
            # 팔레트에서 범주의 수에 맞는 색상값 추출
            colors = sb.color_palette(palette, n_colors=len(categories))
...
def histplot(data=None, x=None, bins="auto",
             title=None, xlabel=None, ylabel=None, hue=None,
...
# 박스플롯 모듈화
# ------------------------------------
def boxplot(data=None, x=None, y=None, hue=None,
            title=None, xlabel=None, ylabel=None, palette=None,
            orient=None, order=None, width=1280, height=640, save_path=None, ax=None):
    # 그래프 초기화
    fig = None
    if ax is None:
...
def heatmap(data=None, annot=True, fmt="0.2f", linewidth=0.5, palette=None, title=None, xlabel=None, ylabel=None, 
    ...
    # 히트맵 그리기
    sb.heatmap(data=data, annot=annot, fmt=fmt, linewidth=linewidth, cmap=palette, ax=ax, **params)
...
def pieplot(x, labels, autopct="%0.1f%%", startangle=90, counterclock=False, 
            explode=None, donutchart=False, wedge_width=0.7, wedge_color="#ffffff", wedge_linewidth=3,
            palette=None, title=None, xlabel=None, ylabel=None,
            width=1280, height=640, save_path=None, ax=None):
    ...
    if text:
        for j, val in enumerate(df[col]):
            ...
            if orient == 'v':
                ...
                ax.text(x=df.iloc[j, :i].sum() + val / 2, y=j, s=text_format.format(val), ha='center', va='center',
                        color=text_color, fontsize=text_fontsize)
...
def plot_hull(data, x, y, hue, palette, ax):
    ...
    for i, v in enumerate(classes):
        ...
        dfc = data.loc[data[hue] == v, [x,y]]

        # ConvexHull은 3개 이상의 점이 필요하므로, 데이터 포인트가 3개 미만인 경우 중단해야함
        if len(dfc) < 3:
            continue
        ...
        ax.plot(dfc.iloc[points, 0], dfc.iloc[points, 1], linewidth=1, linestyle=":", color=color)
        ax.fill(dfc.iloc[points, 0], dfc.iloc[points, 1], alpha=0.1, color=color)
...
def lmplot(data, x, y, hue=None, palette=None, col=None, row=None, markers="o",
    ...
    if not hue and palette:
        ...
        linecolor = None
    # 2) lmplot 그리기
    g = sb.lmplot(data=data, x=x, y=y, height=h, aspect=w/h, hue=hue, col=col, row=row, legend=False, markers=markers, palette=palette,
                  scatter_kws={
...
def pairplot(data, x=None, y=None, hue=None, palette=None, diag_kind="kde", reg=False,
             markers="o", scatter_size=20, scatter_alpha=0.8, linecolor=None, linewidth=1.5, linestyle="-", 
...
def pointplot(data, x = None, y = None, hue = None, errorbar='se', title=None, xlabel=None, ylabel=None,
    ...
    # 점 그래프 그리기
    sb.pointplot(data=data, x=x, y=y, hue=hue, order=order, hue_order=hue_order, estimator=estimator,
                 errorbar=errorbar, capsize=capsize, dodge=dodge, markers=markers, linestyles=linestyles,
                 palette=palette, color=color, ax=ax)
    ...
    if fig is not None:
        show(save_path=save_path)
# ------------------------------------------------------------------------------------
# 그래프 캔바스 객체(ax)에 신뢰구간을 표시하는 보조함수 정의
# ------------------------------------------------------------------------------------

def _draw_ci(ax, interval, color, ymax):
    """
    kdeplot에서 단일 신뢰구간 (하한~상한)을 지정한 색상으로 그리는 보조함수
