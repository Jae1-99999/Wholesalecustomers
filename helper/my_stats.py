...
from math import sqrt
from pandas import DataFrame, melt , Series, concat, crosstab
from scipy.stats import t
...
def posthoc_oneway(data, y, between, alpha=0.05, plot=True, palette=None,
                   ...
                   height=640, summary=True, save_path=None) :
...
def anova_twoway(data, y, between, alpha=0.05, order=None, plot=True, palette=None,
    ...
    """이원분산분석 ( Two-way ANOVA )
        ...
        - 등분산 미충족 : Type-2 ANOVA
        
        
    Args:
        data (DataFrame) : 검정 대상 데이터 프레임 (long출력)
        y (str) : 종속변수(연속형) 컬럼명
        between (list) : 집단을 구분하는 두개의 독립변수 (명목형) 컬럼명 리스트
        alpha (float) : 유의수준 (기본값 : 0.05)
        ...
        width (int) : 서브픞ㄹ롯 한 칸의 너비 (기본값 : 640)
    ...
    if equal_var :
        ...
        # [등분산 미충족] 등분산을 가정하지 않은 선형모형(HC3) 기반 Type-Ⅱ ANOVA
        test_name = "Type-Ⅱ ANOVA"
        ...
        # statmodels 결과표에는 np2가 없으므로 편에타제곱을 직접 계산
        ...
        # statsmodels가 만든 효과명(C(Q('water')):C(Q('sum')) 등)을
        # pingouin 결과와 동일한 형태(water * sum)로 정리한다.
    ...
    # 어떤 감정을 사용했는지 식별할 수 있도록 맨 앞에 test 컬럼 추가
    ...
    labels = ["Large", "Medium", "Small"]
    aov["effect_size"] = np.select(conditions, labels, default="Negligible")
    # np2가 없는 행(잔차 등)은 효과크기 해석 대상이 아니므로 표시 비움
...
# 이원분산분석 사후검정
# -----------------------------------------------------------------------------------
def posthoc_twoway(data, y, between, alpha=0.05, plot=True, summary=True, palette=None, title=None, xlabel=None, ylabel=None,
                   width=1280, height=640, save_path=None):
    ...
    Args:
        ...
        alpha (float) : 유의수준 (기본값 : 0.05)
        plot (bool) : 상호작용 플롯을 시각화할지 여부 (기본값 : True)
        summary (bool) : 결과요약(비교쌍 수, 유의한 쌍 수, 평균차가 가장 큰/작은 쌍,
                          첫번째 요인의 수준별 유의한 쌍)을 출력할지 여부 (기본값 : True)
        palette (str or list) : 색상 팔레트 (기본값 : None)
        title (str) : 그래프 전체 제목 (기본값 : None)
        ylabel (str) : y축 라벨 (기본값 : None이면 "평균 (y)")
        width (int) : 서브픞ㄹ롯 한 칸의 너비 (기본값 : 640)
    ...
    Returns:
        DataFrame : 조합(셀) 집단 쌍별 사후검정 결과표(Tukey HSD OR Games-Howell)
    """

    # 1) 분석에 사용할 컬럼만 추출하고 결측 행 제거
    df = data[[y, between[0], between[1]]].dropna().copy()

    # 2) 두 명목형 변수 결합하여 조합(셀) 단위의 집단 컬럼 생성
    ...
    # hedges는 두 집단 평균차에 대한 표준화 효과크리고,
    # Cohen의 d 기준표를 따라 절댓값으로 해석한다.
    #   ≥ 0.8 -> 큼, ≥ 0.5 -> 중간, ≥ 0.2 -> 작음, 그미만 -> 미미함
    abs_hedges = result["hedges"].abs()
    conditions = [
        abs_hedges >= 0.8,
        abs_hedges >= 0.5,
    ...
    if plot:
        ...
        order = (df[[group, y]].groupby(group).mean().sort_values(y, ascending=False).index.tolist())
        ...
        my_plot.boxplot(data=df, x=group, y=y, order=order, palette=palette,
                        title=title, xlabel=xlabel, ylabel=ylabel, width=width, height=height, save_path=save_path)
    ...
    if summary:
        ...
        big = result.loc[result["diff"].abs().idxmax()]
        print("평균 차이가 가장 큰 쌍: ({0}) vs ({1})"
              "-> 차이 {2:.3f}, 효과크기 {3}, p-value {4:.3f}".format(big["A"], big["B"], abs(big["diff"]), big["effect_size"], big["pval"]))
        
        sm = result.loc[result["diff"].abs().idxmin()]
        print("평균 차이가 가장 작은 쌍: ({0}) vs ({1})"
              "-> 차이 {2:.3f}, 효과크기 {3}, p-value {4:.3f}".format(sm["A"], sm["B"], abs(sm["diff"]), sm["effect_size"], sm["pval"]))
        
        # 첫 번째 요인의 수준을 고정한 채 두 번째 요인 끼리만 비교한 쌍을 따로 살핀다.
        ...
        for lv in df[between[0]].unique():
            ...
            if sig.empty:
                # 이수준에서는 두 번째 요인을 무엇으로 바꿔도 차이를 확인할 수 없다.
                print("(유의한 쌍 없음)")
...
def correlation(data, x, y, alpha=0.05, plot=True, palette=None, title=None, xlabel=None, ylabel=None, width=1280, height=640, save_path=None):
    ...
    # H0 : 모형이 올바르게 설정된(선형). p >= alpha이면 선형성 충족.
    ...
    use_pearson = linearity and norm_x and norm_y and (not influential) and (not high_skew)
    ...
    # 6) 상관 강도 해석 라벨
    a = abs(coef)
    if a > 0.7 :        strength = "강함"
    elif a > 0.3 :      strength = "중간"
    elif a > 0 :        strength = "약함"
    else:               strength = "관계없음"

    # 7) 가정 점검 결과와 선택된 상관계수를 단일 행 결과표로 정리
    row = {
        "x" : x, "y" : y, "method" : method, "coef" : round(float(coef), 4), "strength" : strength, "significant" : bool(p < alpha),
        "normality" : norm_x, "normality_y" : norm_y, "linearity" : linearity, "influential_outlier" : influential, "high_skew" : high_skew,
    }
    ...
    if plot:
        my_plot.scatterplot(data=pair, x=x, y=y, palette=palette, title=title, xlabel=xlabel, ylabel=ylabel, width=width, height=height, save_path=save_path)
...
def multi_correlation(data, columns=None, alpha=0.05, plot=True, palette=None, diag_kind="kde", reg=False, title=None, width=1280, height=1024, save_path=None):
    ...
    Args:
        ...
        palette (str or list) : 색상 팔레트 (기본값:None)
        diag_kind (str) : 산점도 행렬 대각선 그래프 종류 'hist' OR 'kde' (기본값: "KDE")
    ...
    display(corr_matrix.style
            .background_gradient(cmap="coolwarm", vmin=-1, vmax=1).format("{:.3f}"))
    
    # 4) 산점도 행렬 시각화
    if plot:
        my_plot.pairplot(data=data[columns], palette=palette, diag_kind=diag_kind, reg=reg, title=title,
...
def chi2_goodness_of_fit(data, column, expected=None, order=None, alpha=0.05,
    ...
    Args:
        ...
        expected (list | None) : 각 범주의 기대반도 또는 기대비율. None이면 균등분포 (기본값:None)
        order : 명목형 범주의 표시·계산 순서를 지정하는 리스트
        alpha (float) : 유의수준(기본값:0.05)
    ...
    # 3) 가정 확인
    # 기대빈도 점검 (5이상 셀이 80% 이상 + 1 미만 셀 없음)
...
# 독립성/동질성 일괄 처리 함수 정의
# ---------------------------------------------------------------------
def _chi2_crosstab(data, row, col, kind, alpha=0.05, plot=True, palette=None, orient='v',
                   title=None, xlabel=None, ylabel=None, width=1280, height=640, save_path=None):
    ...
    Args:
        ...
        plot (bool) : 결과를 시각화할지 여부 (기본값: True)
        palette (str or list): 색상 팔레트 (기본값: None)
        orient (str) : 그래프 방향 ('v' OR 'h', 기본값: 'v')
        title (str) : 그래프 제목 (기본값:None)
    ...
    # 2) 가정 확인
    # 카이제곱으로 기대빈도 확보 (2x2는 에이츠 보정 기본 적용)
...
def chi2_independence(data, x, y, alpha=0.05, plot=True, palette=None, orient='v',
                    title=None, xlabel=None, ylabel=None, width=1280, height=640, save_path=None):
...
# --------------------------------------------------------------------------
# 상관분석 결과표를 통해 다중공선성 신호를 감지하는 함수 정의
# --------------------------------------------------------------------------

def correlation_summary(corr_df, strength='강함'):
    ...
    # -> 함수 호출 시 ["강함", "중간"] 같이 여러 강도를 지정할 수도 있도록 하기 위함
    if type(strength) == str:
        strength = [strength]

    # 강도로 걸러야 변수마다 개수가 달라져 중복의 정도를 구분할 수 있다.
    group_pairs = (pairs[pairs["strength"].isin(strength)].groupby("x")["y"].agg(["count", ", ".join]))

    # 4) 두 결과를 병합
    # 중복 쌍이 없는 변수는 병합 결과가 결측이 되므로 0과 "-"로 채운다.
    corr_table = summary[["max-y", "max-coef"]].merge(group_pairs, left_index=True, right_index=True, how="left")
