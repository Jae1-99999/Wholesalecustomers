...
def fit_model(data, y, summary = False):
    ...
    if summary:
        print(fit.summary())        # 적합된 모델의 요약 통계량 출력여부 확인
...
def predict(fit,new_data):
    """적합된 회귀모델을 이용해 새로운 데이터에 대한 예측값을 계산한다.
        ...
        Args :
            ...
            new_data: 예측에 사용할 새로운 데이터프레임. 독립변수 컬럼만 포함해야한다.
...
# 선형성 가정 검정 함수 정의
# ------------------------------------------------------------------------
def test_linear(fit, alpha=0.05, plot=True, palette=None, title=None, xlabel=None, ylabel=None, width=1280, height=640, save_path=None):
    """잔차의 선형성(모형 설정 오류)을 검정한다.
    
    Ramsey RESET Test(power=2)를 수행하여 적합된 선형모형에 고차항을 추가했을 때 유의미한 설명력이 남는지를 확인한다.
    고차항이 유의하면 (=p < alpha) 직선으로는 잡아내지 못한 곡선 관계가 남아 있다는 뜻이므로 선형성 가정이 위배된다.
    ...
    if plot:
        # 팔레트가 지정되면 첫 번 째 색을 산점도 점 색상으로 사용
        ...
        fig, ax = my_plot.init(width=width, height=height, title=title,
                               ...
                               ylabel=ylabel if ylabel else "잔자(residual)")

        # 잔차 = 0 기준선(파란 점선)
        my_plot.lineplot(x=[plot_df["y_pred"].min(), plot_df["y_pred"].max()], y=[0, 0], color="blue", linestyle="--", ax=ax)
...
def test_normal(fit, alpha=0.05, plot=True, palette=None, width=1280, height=640):
    ...
    Args:
        ...
        palette (str) : 그래프 색상에 사용할 팔레트 이름. None이면 기본색을 사용한다 (기본값:None)
        width (int) : 그래프 너비 (기본값:1280)
        height (int) : 그래프 높이 (기본값:640)
    """
    ...
    if plot:
        ...
        # Q-Q 플롯 그리기
        my_plot.lmplot(
            data=qq_df, x="qq_x", y="qq_y", linecolor=line_color, linestyle="--",
    ...
    # 잔차 표준편차 추정치√MSE를 기준으로 ±1·±2·±3√MSE 구간의 실제 포함 비율을 구하고,
    ...
    for k ,exp in zip((1,2,3), expected):
        ...
        mse_rows.append({                   # 구간별 판정 상세 내용을 딕셔너리로 구성
            "구간" : f"±{k}√MSE",
            "기대(%)" : round(exp * 100, 1),
...
def report_fitness(fit, log_y=False, log_x=None, log1p_y=False, log1p_x=None, reflect_y=False, reflect_x=None):
    """적합된 회귀모델의 모형 적합도(model fit)를 학술 보고 형식의 문장으로 생성해 반환한다.
    
    summary() 결과표의 문자열을 파싱하지 않고, 'fit' 객체가 이미 갖고 있는 속성에서
    지표를 직접 읽어 문장을 구성한다. 표에 보이는 값은 반올림된 표시값이지만
    ...
    Args:
        ...
        log_x (list | None) : log변환을 적용한 독립변수 이름 리스트 (기본값: False)
        log1p_y (bool) : 종속변수에 log1p(=ln(1+y)) 변환을 적용했는지 여부 (기본값: False)
        log1p_x (list | None) : log1p변환을 적용한 독립변수 이름 리스트 (기본값 : None)
    ...
    # 반사 변환은 log1p와 식이 다르므로(대소가 뒤집힌다) 라벨도 구분해 적는다.
    ...
    report = template.format(
        ...
        alpha=alpha,
        r_squared=(fit.rsquared, 3),
        adj_r_squared=(fit.rsquared_adj, 3),
...
def report_variables(fit, data, hc3=False):
    ...
    # cov_type 지정 시 로버스트 표준오차·t·유의확률을 별도로 계산
    ...
    for x in xnames:
        ...
        if hc3:
            # 로버스트 비교형식 : B(양쪽공유) + 일반(SE·t·유의확률) + 로버스트(SE·t·유의확률)를
...
# ----------------------------------------------------------
# 회귀계수 보고 문장 생성 함수 - 함수 정의
# ----------------------------------------------------------
def report_variables_text(fit, log_y=False, log_x=None, log1p_y=False, log1p_x=None, reflect_y=False, reflect_x=None, hc3=False):
    ...
    Args:
        ...
        hc (bool) : True이면 HC3 로버스트 표준오차를 사용한다. Defaults to False.
    ...
    # 상수항(const)를 제외한 독립변수 이름 (위치가 아니라 이름으로 걸러낸다)
    xnames = []
    for name in fit.model.exog_names:
        if name != "const":
            xnames.append(name)
    ...
    # 반사 변환이 끼면 위 문장은 반사값 기준이므로, 원 변수 기준의 방향을 깗게 덧붙인다
    ...
    for x in xnames:
        ...
        if not x_pct and not y_pct:     # 원본 → 정대량 그대로
            mag, unit, approx = f"{abs(B):.2f}", "", ""
        elif x_pct and not y_pct:       # 독립변수만 로그 → 1% 증가당 절대 변화 = Bxln(1.01)
            mag, unit, approx = f"{abs(B * np.log(1.01)):.3f}", "", "약 "
        elif not x_pct and y_pct:       # 종속변수만 로그 → (e^B - 1)x100 %
...
def auto_ols(data, y, report=True,
    ...
    while True:
        ...
        if len(pvalues) <= 1 or pvalues.max() < alpha:      # 후진소거법의 반복 종료 조건
            break
...
def plot_beta(fit, data, palette=None, title=None, xlabel=None, ylabel=None, width=1280, height=None, save_path=None):
    ...
    막대는 'report_variables'가 정렬해 둔 |β| 내림차순 그대로 위에서 아래로 배치되며, 계수의 부호게 따라 색을 달리하고 막대 끝에 β 값을 표기한다.
    ...
    Args:
        ...
        height (int) : 캔버스 세로 픽셀. None이면 독립변수 수 x 80으로 자동계산 (기본값:None)
        save_path (str) : 이미지 저장 경로 (기본값:None)
    """

    # 1) 시각화용 데이터 전처리
    # 회귀계수 표 리턴받기 - 베타값 자체는 hc3 여부와 무관하므로 h3=False로 호출한다.
...
def fit_pipeline(data, y, nominal_cols=None, *,
    ...
    Args:
        ...
        save_path (str) : 전처리 완료 데이터의 저장 경로 (.xlsx / .xls / .csv )
    ...
    if nominal_cols is None:
        ...
        for c in nominal_cols:
            if c not in df.columns:         # <- 선전된 명목형 변수목록이 실제 데이터 프레임에 없다면 없는 항목만 missing 리스트에 담는다
                missing.append(c)

        if missing:                         # 결과로 missing에 담겨있는 항목이 있으면 존재하지 않는 컬럼이 지정된 경우로 간주
    ...
    # 순수 log와 log1p는 % 해석의 기준이 다르므로(전자는 변수, 후자는 1+변수) 따로 관리한다
    log_y = False
    log_x = []
    log1p_y = False
    log1p_x = []

    # 반사(좌측 꼬리) 변환 정보. 해석의 기준·방향이 log1p와 다르므로 따로 관리한다.
    ...
    if log:
        ...
        # 우측 꼬리는 값의 위치에 따라 log와 log1p로 갈리고, 좌측 꼬리는 반사 후 log1p로 구분
        log_columns = []
        log1p_columns = []
        reflect_columns = []
    ...
    if vif and continuous:
        ...
        df = my_prep.reduce_vif(df, columns=continuous, threshold=vif_threshold, verbose=verbose)
    ...
    if scale and continuous:
        ...
        df = my_prep.scaling(df, columns=continuous, method=scale_method, verbose=verbose)
    ...
    # 13-1) 숫자로 바뀌지 않은 컬럼 확인 (남아 있으면 OLS가 알 수 없는 오류를 낸다)
    remain = []
    for c in df.columns:
    ...
    # 13-2) 결측치 확인 (남아 있으면 OLS가 MissingDataError를 낸다)
    ...
    # 14) 모델 적합
    fit = auto_ols(df, y, backward=backward, alpha=alpha, log_y=log_y, log_x=log_x, log1p_y=log1p_y, log1p_x=log1p_x,
                   reflect_x=reflect_x, reflect_y=reflect_y, report=False, test=False)
    ...
    # 전처리가 끝난 데이터. report_variables(), plot_beta() 가 β·VIF 계산에 사용해야한다.
    fit.data_ = df

    # 모델을 구분할 이름 (compare_models가 딕셔너리 키로 채워주기도 한다)
    fit.name_ = name
...
def compare_models(fits, metric='RMSE', sub_metric='변수수', tolerance=0.05,
    ...
    Raises:
        ...
        ValueError: 'fits'가 비었거나 지표 이름·tolerance가 유효하지 않는 경우.
    """
    ...
    # (모델에 들어간 값 그대로 계산되므로 로그변환 여부가 다르면 값의 단위 자체가 다르다)
    ...
    for name, fit in fits.items():
        ...
        # -> getattr()로 속성이 없으면 False/None을 반환하도록 한다
        ...
        # 3-2) 원 척도로 되돌리기 (로그변환 여부에 따라 다르다)
        if plain_log_y:
            # log(y)의 역변환: exp(y)
    ...
    # 최족값이 0이면 나눌 수 없으므로 격차를 값의 차이 그대로 본다
    if best != 0:
    ...
    if sub_metric:
        ...
        ascending.insert(0, not metrics[metric])
    ...
    # 각 모델에 딕셔너리 키를 이름으로 새겨둔다 (직접 지정한 name_ 이 없을 때만)
...
def report_model(fit, title=True, plot=True):
    ...
    'fit_pipeline'·'auto_ols'가 결과 객체에 붙여 둔 정보('log1p_y_'·'log1p_x_'·'user_hc3_'·'data_')를 사용하므로, 이 값들을 따로 준비해 넘길 필요가 없다.
    ...
    if missing:
        raise AttributeError(
            ...
            f'report_model은 fit_pipeline() 또는 auto_ols()로 적합한 모델에 사용하세요.')
    ...
    # 순수 log와 반사 변환정보. 이 속성이 없는 예전 결과 객체도 그대로 동작하도록 기본값을 둔다
    ...
    heading("#### 1) 모형 적합도")
    display(Markdown(report_fitness(fit, log_y=log_y, log_x=log_x, log1p_y=log1p_y, log1p_x=log1p_x, reflect_y=reflect_y, reflect_x=reflect_x)))
    ...
    display(Markdown(report_variables_text(fit, log_y=log_y, log_x=log_x, log1p_y=log1p_y, log1p_x=log1p_x, reflect_y=reflect_y, reflect_x=reflect_x, hc3=hc3)))
