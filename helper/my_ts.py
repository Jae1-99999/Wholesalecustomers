...
def adf_test(data, column=None, name=None, alpha=0.05):
    ...
    여러 대상을 비교하는 adf_diff · adf_transform도 이 함수를 반복 호출한다.
...
def adf_diff(data, column=None, max_diff=-1, alpha=0.05):
    ...
    best = result_df["표준편차"].idxmin()                # 표준편차가 최소인 차수이름
    ...
    for position, name in enumerate(result_df.index):    # 각 행을 순서대로 순회하며
        if position == best_at:                          # 표준편차가 최소인 차수라면
            ...
            recommend.append("과대차분")                 # 차분이 지나쳤다는표시
        else:                                            # 최소점 이전이라면
...
def adf_transform(data, column=None, period=None, max_diff=2, log=True, alpha=0.05):
    ...
    Returns:
        DataFrame : 조합별 ADF 검정 결과표. 정상성과 표준편차 순으로 정렬된다.
    """
...
# ---------------------------------------------------------
# 원본 + 이동평균 + 이동표준편차 시각화 함수 정의
# ---------------------------------------------------------
def plot_rolling(data, window, column=None, title=None, xlabel=None, ylabel=None, width=1280, height=480, save_path=None):
...
def compare_smoothing(data, sizes, column=None, method="ma", overlay=False,
    ...
    # 1) 창 크기별 평활 계산
    if column is not None:                  # 컬럼명이 전달 되었다면
        data = data[column]                 # 대상 컬럼만 추출
    ...
    # 3) 격자에 한 칸씩 그리기
    cols = 2
    rows = int(np.cell(len(result_df.columns) / cols))
...
def decompose(data, period, column=None, model=None, plot=True, title=None,
    ...
    Args:
        ...
        period (int) : 계절 주기
        column (str) : data가 데이터프레임인 경우 대상 컬럼명 (기본값 : None)
        medel (str) : "additive" 또는 "multiplicative". None이면 자동 판정 (기본값 : None)
        plot (bool) : 성분별 그래프를 그릴지 여부 (기본값 : True)
        title (str) : 그래프 제목 (기본값 : None)
        rows (int) : 격자 행수 (기본값 : 2)
        cols (int) : 격자 열수 (기본값 : 2)
        width (int) : 캔버스 가로 픽셀 (기본값: 960)
    ...
    if plot:
        ...
        for i, col in enumerate(result_df.columns):
            if col == "잔차":
                ...
                my_plot.scatterplot(data=None, x=result_df.index, y=result_df[col], size=8, linewidth=0.5, ax=ax[i])
...
# 계절 지수표 생성
# --------------------------------------------------
def report_seasonal(data, period, column=None, model=None, plot=True, title=None, xlabel=None, ylabel=None,
    ...
    Returns:
        DataFrame : 주기 내 위치 (1~period)를 인덱스로 하는 계절 지수표.
    """
...
def adjust_seasonal(data, period, column=None, model=None, plot=True, title=None,
    ...
    """원본에서 계절 성분을 걷어내 계절 조정 시계열을 만든다.
...
def auto_correlation(data, column=None, period=None, plot=True,
    ...
    """ACF와 PACF를 한 번에 계산해 유의성과 차수 후보(p·q)를 판정한다.
    ...
    Args:
        ...
        width (int) : 캔버스 가로 픽셀 (기본값: 1280)
        height (int) : 캔버스 세로 픽셀 (기본값 : 380)
        title (str) : 그래프 제목 (기본값: None)
        save_path (str) : 이미지 저장 경로 (기본값 : None)
        
    Returns:
        DataFrame : lag별 acf·pacf·유의성과 p·q 위치 표시.
            p와 q 중 뒤쪽 lag까지만 잘라서 반환한다.
    """
    # 1) 대상 추출
    if column is not None:
        data = data[column]
    ...
    acf_values = acf(data, nalags=lags)[1:]
    ...
    # ACF의 절단 → MA 차수(q), PACF의 절단 → AR차수 (p)
    ...
    if period is not None:
        # 계절 차수 N은 lag N*period에서 확정된다. (차수 0이면 표시할 자리가 없다)
    ...
    for name, code, order, kind, step, in (("AR", "p", p, "PACF", 1), ("MA", "q", q, "ACF", 1)):
        reason = (f"{kind}가 lag {order * step} 이후 절단" if order else f"{kind}가 lag {step}부터 비유의")
        print(f"{name} 차수 후보 ({code}): {order}      ← {reason}")
    if period is not None:
        for name, code, order, kind, step in (("계절 AR", "P", P, "계절 PACF", period), ("계절 MA", "Q", Q, "계절 ACF", period)):
            reason = (f"{kind}가 lag {order * step} 이후 절단" if order else f"{kind}가 lag {step}부터 비유의")
            print(f"{name} 차수 후보 ({code}): {order}          ←{reason}"
                  f"(판정 시차 {season_lags})")
...
def fit_model(data, column=None, p=1, d=1, q=1, P=1, D=1, Q=1, s=None,
    ...
    for try_p, try_d, try_q, try_P, try_D, try_Q in orders:
        # 3-1) 적합 시도
        try:
            # 조합을 여러개 적합하면 수렴 경고가 반복 출력되므로 이 안에서만 숨긴다
...
def predict(fit, steps, alpha=0.05, plot=True, history=None,
    ...
    Args:
        fit: 'fit_model' 함수로 적합된 SARIMAX결과 객체.
    ...
    # 적합에 쓴 원본은 결과 객체가 그대로 들고있다. 따로 넘겨받을 필요가 없다
    ...
    my_plot.lineplot(x=linked.index, y=linked.values, label="예측값", linestyle="--", ax=ax)

    # 예측구간은 직선이 아니라 점선으로 그린다. 뒤로 갈수록 넓어지는 모습이 드러난다
    ax.fill_between(result.index, result["lower"], result["upper"], alpha=0.2, label=f"{(1-alpha) * 100:.0f}% 예측구간")
...
def report_residual(fit, alpha=0.05, report=True, plot=True, title=None,
    ...
    """
    # 1) 모형에서 차수 정보를 꺼낸다
    # SARIMAX 계열은 (p,d,q)와 (P,D,Q,s)를 모형 객체가 그대로 들고있다.
    ...
    # 모형이 추정한 계수 수만큼 자유도를 빼야한다. 시차가 이보다 작으면 검정이 성립하지 않는다
    model_df = p + q + P + Q
    ...
    if not lags: las = [max(model_df + 1, len(x) // 5)]
    ...
    result.append({
        ...
        # H > 1이면 예측구간이 좁게(낙관적), H < 1 이면 넓게(보수적) 잡힌다
    ...
    # 7) 잔차의 모습 살펴보기
    # 검정 결과는 숫자 하나로만 말해준다. 눈으로 봐야 잡히는 문제도 있다.
...
def report_score(fit, name="적합값", report=True):
    ...
    "작년 같은 달과 똑같을 것이다" 수준의 기준선을 함께 계산해 비교대상으로 삼는다.
    ...
    actual = endog if isinstance(endog, Series) else \
        Series(np.asarray(endog).ravel(), lendex=fit.fittedvalues.index)
    ...
    for target in table.columns[1:]:            # 0번째 컬럼은 관측값이므로 제외
        ...
        result.append({
            ...
            "RMASE" : round(np.sqrt(np.mean(error ** 2)), 3),
