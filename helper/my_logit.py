...
def fit_model(data, y, summary=False):
    ...
    종속변수는 0/1의 두값만 가지는 이분형이어야 한다.
    
    Args:
        data : 독립변수와 종속변수를 모두 포함하는 데이터프레임.
        y : 종속변수로 사용할 컬럼명. 'data'에 반드시 존재해야 하며 0/1의 이분형이여야 한다.
        summary : True로 설정하면 적합된 모델의 요약 통계량을 출력
                    Defualt to False.
    ...
    model = Logit(y_series, x_input)        # 로지스틱 모델 객체 생성
    fit = model.fit(disp=0)                 # 모델 적합. disp=0 -> 수렴 메세지 출력안함
...
def predict(fit, new_data, threshold=0.5):
    ...
    로지스틱 회귀의 예측값은 '1'(사건 발생)일 확률이므로, 임계값(Threshold)을 초과하면 1, 그렇지 않으면 0으로 분류한다.
    ...
    Returns:
        DataFrame : 예측확률('proba')과 예측값('pred')을 담은 데이터프레임
    """
    # 새로운 데이터에 절편(상수항) 추가
    ...
    # 예측 확률과 임계값 기준 예측 범주를 DataFrame으로 변환
    return DataFrame({
        "proba" : proba,                            # 1이 될 확률
...
# 시그모이드 곡선 시각화 함수 정의
# -------------------------------------------
def plot_sigmoid(fit, data, x, threshold=0.5, palette=None, title=None, xlabel=None, ylabel=None, width=1280, height=640, save_path=None):
    ...
    # 그래프 초기화 및 축 레이블 설정
    fig , ax = my_plot.init(width=width, height=height, title=title, xlabel=xlabel if xlabel else x,
                            ylabel=ylabel if ylabel else f"P({yname}=1)")
    
    # 4) 실제 관측치(0/1) 산점도
    # 같은 높이(0 또는 1)에 점이 겹쳐 보이므로 반투명하게 처리한다
    my_plot.scatterplot(data=data, x=x, y=yname, color="#888888", alpha=0.4, palette=None, ax=ax)
    ...
    ax.text(x=boundary, y=threshold, s=f"분류 경계: {boundary:.2f}", color="red", va="bottom", ha="left")
...
def report_fitness(fit):
    """적합된 로지스틱 모델의 적합도(model fit) 보고 문장을 생성해 반환한다.
...
def report_variables(fit, data):
    ...
    Args:
        fit: 'fit_model' 함수로 적합된 회귀분석 결과 객체.
        data: 회귀분석에 사용한 원본 데이터 프레임. 독립변수와 종속변수를 모두 포함해야한다.
    ...
    params = np.asarray(fit.params)            # 비표준화 회귀계수(β)
    ...
    for x in xnames:
        ...
        # 표준화 회귀계수
        # --> (βstd) = B x 독립변수 표준편차 = "독립변수 1 SD 변화당 log(오즈비) 변화".
...
def report_variables_text(fit, data=None, alpha=0.05):
    ...
    line_template = (
        ...
        "(**z = {z}**, **{p}**) "
        "즉, {change} {y}가 1(사건 발생)이 될 오즈는 평균적으로 약 **{pct}%**"
        "**{direction}**하는 것으로 해석된다."
    )
...
def plot_odds(fit, data, palette=None, title=None, xlabel=None, ylabel=None,
    ...
    Args:
        ...
        width (int) : 캔버스 가로 픽셀 (기본값 : 1280)
        height (int) : 캔버스 세로 픽셀. None이면 독립변수 수 x 80 으로 자동 계산 (기본값: None)
        save_path (str) : 이미지 저장 경로 (기본값: None)
    """
    ...
    # 2) 그래프 초기화
    fig, ax = my_plot.init(width=width, height=height, title=title, xlabel=xlabel if xlabel else "오즈비(Odds Ratio)",
    ...
    for i in rdf.index:
        ...
        ax.text(x=orv, y=i, s=f"{orv:.2f}", va="center", ha="left" if orv >= 1 else "right",
                color="black")
...
# ----------------------------------------------------------------------------
# 로지스틱 회귀모형 적합 + 후진 소거 자동처리 + 결과보고 자동화 함수
# ----------------------------------------------------------------------------
def auto_logit(data, y, report=True, plot=True, threshold=0.5, width=1280,
               height=640, backward=False, alpha=0.05):
    """로지스틱 회귀모델 적합부터 변수 선택·보고서 출력·시각화까지 한번에 수행한다.
    
    'backward=True'이면 유의하지 않은 독립변수가 모두 사라질 때까지 후진소거법
    (유의확률이 가장 큰 변수를 하나씩 제거 후 재적합)을 반복한다.
