...
def best_k_elbow(data, klist=None, columns=None, scaling='standard', sensitivity=0.01, random_state=RANDOM_STATE, n_init=10, verbose=True,
    ...
    # np.diff()는 바로 앞 값과의 차이를 구해준다 (이너셔는 줄어드므로 음수 → 절대값으로 변환)
    ...
    # 4) 엘보우 포인트 찾기
    # convex(아래로 블록) + decreasing(우하향)은 이너셔 곡선의 모양이다
    kl = KneeLocator(klist, inertia, curve='convex', direction='decreasing', S=sensitivity)
    best_k = kl.elbow

    # 곡선이 거의 직선이면 꺾이는 지점이 없어 None이 나온다
    if best_k is None:
        print("엘보우 포인트를 찾지 못했습니다. "
              "sensitivity를 낮추거나 klist의 범위를 넓혀 다시 시도해 보세요.")
    ...
    if plot:
        ...
        my_plot.lineplot(x=klist, y=inertia, color=color, marker=marker, linestyle=linestyle, ax=ax)
...
# ---------------------------------------------------------------
# 실루엣 스코어와 면적 두께를 함께 평가해 최적 k찾는 함수정의
# ---------------------------------------------------------------
def best_k_silhouette(data, klist=None, columns=None, scaling='standard', random_state=RANDOM_STATE, n_init=10, verbose=True,
                      plot=True, plot_each=False, title=None, palette='tab10', width=800, height=580, save_path=None):
    """실루엣 스코어와 면적 두께를 함께 평가해 최적의 k를 추정하는 함수
    ...
    Args (기본값은 위의 함수 정의 참고) : 
        ...
        title, palette: 그래프 제목(None이면 군집 개수와 스코어로 자동생성),
    ...
    for k in klist:
        ...
        total_area = area.sum()             # 계수의 합 = 데이터 개수 x 스코어
        ...
        items.append({
            ...
            "균형지수": round(thickness * min_area / thickness, 4),
        })

        # 1-5) k마다 실루엣 막대와 산점도를 확인 (눈으로 면적·두께를 함께 보기 위한 옵션)
        # k마다 실루엣 막대와 산점도를 확인 (눈으로 면적·두께를 함께 보기 위한 옵션)
        if plot_each:
            visualize_silhouette(estimator, df, palette=palette, width=width, height=height)
    ...
    # (plot_each로 이미 모든 k를 그렸다면 같은 그림을 두 번 그리지 않는다)
    if plot and not plot_each:
        visualize_silhouette(estimators[best_k], df, title=title, palette=palette, width=width, height=height, save_path=save_path)
...
# 덴드로그램 시각화
# ---------------------------------------------------------------
def plot_dendrogram(estimator, cut_height=None, title=None, p=30, truncate_mode='lastp', leaf_rotation=0, leaf_font_size=8,
                    count_sort='ascending', cut_line=True, cut_color='#ff0000', xlabel=None, ylabel='병합 거리', width=1280,
                    height=640, save_path=None, ax=None):
    """계층적 군집 모델이 합쳐온 과정을 덴드로그램으로 그리는 함수
    
    Args:
        estimator : compute_distances=True 로 학습한 AgglomerativeClustering 모델
        cut_height : 나무를 자른 높이(병합거리). 이 높이 아래의 가지를 군집별로 다른 색으로
            칠하고 가로선으로 표시한다. None이면 모델이 실제로 자른 높이를 직접 계산해서 쓰므로,
            보통은 지정할 필요가 없다
        ...
        leaf_rotation, leaf_font_size, count_sort : 아래쪽 눈금의 회전 각도, 글자크기,
            가지의 정렬 기준('axcending'이면 작은 덩어리를 왼쪽에 둔다)
    ...
    # 가지를 묶어 그리는 경우에만 눈금에 (묵인 개수)가 표시되므로 축 이름을 나눠 쓴다
    if xlabel is None:
...
def agglomerative(data, k=None, distance_threshold=None, columns=None, scaling='standard',
                  ...
                  count_sort='axcending', cut_line=True, cut_color="#ff0000",
    ...
    Args (기본값은 위의 함수 정의 참고):
        ...
        distance_threshold : 병합을 멈출 거리 기준 (이 값 이상 떨어진 덩어리는 합치치 않는다)
        ...
        p, truncate_mode : 덴드로그램에 표시할 가지의 개수와 생략 방식
            ('lastp'=마지막 p개의 가지만, None이면 데이터 전체를 그린다)
        leaf_rotation, leaf_font_size, count_sort : 아래쪽 눈금의 회전각도, 글자 크기,
            가지의 정렬 기준('ascending'이면 작은 덩어리를 왼쪽에 둔다)
        cut_line, cut_color : 나무를 자른 높이를 가로선으로 표시할지 여부와 그 색상
        wiedth, height, save_path, ax : 캔버스 가로·세로 픽셀, 저장 경로,
            그래프를 그릴 Axes 객체(None이면 새로 생성)
    ...
    if plot:
        ...
        plot_dendrogram(estimator, cut_height=cut_height, title=title, p=p, truncate_mode=truncate_mode, leaf_rotation=leaf_rotation,
                        leaf_font_size=leaf_font_size, count_sort=count_sort, cut_line=cut_line, cut_color=cut_color,
...
# DBscan 군집분석 함수 정의
# ----------------------------------------------------------
def dbscan(data, eps=0.5, min_samples=5, columns=None, scaling='standard', cluster_name='그룹번호', vector_name='벡터유형', metric='euclidean',
           n_jobs=-1, verbose=True, plot=True, x=None, y=None, title=None, outline=True, palette='tab10', size=100, edgecolor='#ffffff',
           linewidth=1.5, alpha=1, core_marker='o', border_marker='^', border_size=120, border_alpha=0.5, noise_marker='X', noise_size=150,
           noise_color='#ff0000', noise_edgecolor='#000000', noise_linewidth=1.5, width=1280, height=640, save_path=None, ax=None):
    ...
    Args (기본값은 위의 함수 정의 참고):
        ...
        outline : 군집의 외곽선(ConvsxHull)을 표시할지 여부
    ...
    # 핵심(core) : 반경안에 min_samples 개 이상을 거느린 데이터 (군집의 몸통)
    ...
    # core_smaple_indices_는 "몇 번째 행"인지를 담은 위치 번호이므로 위치로 사용한다
    ...
    if verbose:
        ...
        # 군집이 하나도 만들어지지 않았다면 두 값이 데이터의 밀도와 맞지 않다는 뜻이다
        if n_clusters == 0:
            print("  · 군집이 만들어지지 않았습니다. "
                  "eps를 키우거나 min_samples를 줄여 다시 시도해 보세요.")
        display(summary_df)

    # 7) 군집 결과 시각화
    if plot:
        # 7-0) 컬럼, 제목 설정
        # 축으로 사용할 컬럼 경정 (지정이 없으면 대상 컬럼의 앞에서 두 개)
        if x is None:       x = columns[0]
        if y is None:       y = columns[1]

        # 제목을 지정하지 않은 경우 두 하이퍼파라미터를 포함한 제목을 자동으로 새성
        if title is None:
        ...
        if ax is None:
            fig, ax = my_plot.init(width=width, height=height, title=title, xlabel=x, ylabel=y)
        ...
        if not core.empty:
            my_plot.scatterplot(data=core, x=x, y=y, hue=cluster_name, palette=palette, marker=core_marker,
                                size=size, edgecolor=edgecolor, linewidth=linewidth, alpha=alpha, outline=False, ax=ax)
            
        # 7-4) 외곽 벡터 --> 군집별 색상(동일), 연한 마커
        # (범례에 같은 군집이 두 번 나오므로 범례는 끈다)
        if not border.empty:
            my_plot.scatterplot(data=border, x=x, y=y, hue=cluster_name, palette=palette,
                                marker=border_marker, size=border_size, edgecolor=edgecolor,
                                linewidth=linewidth, alpha=border_alpha, outline=False, ax=ax, legend=False)
        ...
        # (이상치 후보를 바로 찾기 위한 표시)
        if not noise.empty:
            my_plot.scatterplot(data=noise, x=x, y=y, marker=noise_marker, size=noise_size,
                                color=noise_color, edgecolor=noise_edgecolor,
                                linewidth=noise_linewidth, outline=False, ax=ax, label='noise')
...
# 최적 eps값 추정 함수 정의
# ----------------------------------------------------------------------------------
def best_eps(data, min_samples=5, columns=None, scaling='standard', metric='euclidean', n_jobs=-1, sensitivity=1.0, verbose=True,
             plot=True, title=None, color='#1f77b4', linestyle='-', best_color='#ff0000', width=1280, height=640, save_path=None, ax=None):
    """k-distance plot의 꺾이는 지점을 찾아 DBSCAN의 최적 eps를 추정하는 함수
    ...
    Returns:
        tuple : (best_eps, result_df) - eps 후보,
            거리 순위별 k번째 이웃까지의 거리가 담긴 데이터 프레임
    """
    ...
    # 5) 꺾이는 지점(엘보우 포인트)찾기
    # convex(아래로 블록) + increasing(우상향)은 정렬된 거리 곡선의 모양이다
    kl = KneeLocator(range(len(target)), target, curve='convex', direction='increasing', S=sensitivity)
    ...
    if eps is None:
        print("곡선이 완만해 꺾이는 지점을 찾지 못했습니다. "
              "sensitivity를 낮추거나 min_samples를 조정해 다시 시도해 보세요.")
        return None, result_df

    if verbose:
        print(f"[k-distance] min_sample = {k}, 최적의 eps = {eps:.4f}")
        print(f"    · 꺾이는 위치 = {point}번째 데이터 (전체 {len(target)}개 중)")
        print(f"    · 노이즈 후보 = {int((target > eps).sum())}개 (거리가 eps보다 먼 데이터)")
