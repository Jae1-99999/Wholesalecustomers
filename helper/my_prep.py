...
def long2wide(df, hue, values, dropna=True):
    ...
    wide = pivot_table(data=df, index=df.groupby(hue).cumcount(), columns=hue, values=values, dropna=dropna)
    wide.columns.name= None
    wide.index.name = None
    return wide

# ------------------------------------------------------------------------------------
# 다중공선성 제거 기능 추가에 필요한 모듈 참조 및 함수 정의
# ------------------------------------------------------------------------------------

def reduce_vif(df, columns=None, threshold=10.0, verbose=True):
    """
    다중 공선성이 사라질 때까지 VIF가 가장 큰 변수를 하나씩 반복 제거하는 함수
    
    Args:
        df (DataFrame) : 다중공선성을 제거할 변수들이 포함된 데이터 프레임
        columns (list, optional) : 다중공선성을 판단/제거할 컬럼명 리스트.
    ...
    Returns:
        DataFrame : 대상 변수들의 VIF가 모두 threshold 미만이 되도록 일부 변수가
                    제거된 데이터 프레임. 대상이 아닌 컬럼은 원래 순서대로 보존된다.
    """
    ...
    if columns is None:
        ...
        for c in columns:
            ...
            if missing:
                raise KeyError(f'df에 존재하지 않는 컬럼입니다.: {missing}')
            targets = list(columns)
    
    # 컬럼이름의 오름차순으로 정렬
    targets.sort()

    # 대상으로 제외되는 컬럼(종속변수, 명목형 등)은 그대로 보존하기 위해 컬럼이름을 따로 기록
    keep = []
    for c in df.columns:
        if c not in targets:
            keep.append(c)
...
def labeling(df, columns, save_path=None, verbose=True):
    ...
    Args:
        ...
        save_path (str, optinal): 학습된 인코더들을 저장할 pkl 파일 경로
...
def dummies(df, columns, drop_first=True, verbose=True):
    ...
    Args:
        ...
        drop_first (bool) : 각 컬럼에서 만들어진 더미 중 첫 번째를 제외할지 여부 (기본값: True)
            ...
            다중공선성(더미 변수 함정)이 생기므로, 하나를 빼거 기분(reference) 범주로 삼는다
    ...
    # 2) 더미 변환 수행
    # dtype=int 를 지정해 True/False 가 아닌 0/1로 만든다
    ...
    if verbose:
        ...
        for c in skipped:
            print(f'{c} (2종) -> 생략: 이진변수이므로 원래 컬럼을 유지')
...
def replace_outlier(df, columns=None, method='bound', value=None, verbose=True):
    ...
    Args:
        ...
        columns (list, optional): 이상치를 대체할 컬럼명 리스트
            None이면 df 의 수치형 컬럼을 자동으로 선택한다 (기본값:None)
...
def scaling(df, columns=None, method='standard', save_path=None, verbose=True):
    ...
    Args:
        ...
        method (str) : 사용할 스케일러 이름. 대소문자와 뒤의 'Scaler'는 무시하므로
            ...
            - 'standard' :  (x - 평균) / 표준편차 -> 평균0, 표준편차1
            - 'minmax' :    (x - 최소) / (최대 - 최소) -> 0 ~ 1
            - 'robust' :    (x - 중앙값) / IQR -> 이상치의 영향을 덜받음
            - 'maxabs' :    x / |최대| -> -1 ~ 1 (0을 보존하므로 최소 데이터에 사용)
    ...
    # 5) 학습된 스케일러 저장 (선택)
    # 스케일러 안에는 train에서 구한 기준값(평균·표준편차 등)이 들어 있으므로,
    # 이 파일이 있어야 test 데이터에 똑같은 기준을 적용할 수 있다.
    if save_path:
        folder = os.path.dirname(save_path)
        if folder:
            os.makedirs(folder, exist_ok=True)  # 경로에 없는 폴더가 있으면 만듬
...
def replace_missing(df, columns=None, method='mean', value=None, verbose=True):
    ...
    Args:
        ...
        columns (list, optional): 결측치를 처리할 컬럼명 리스트
            None이면 결측치가 있는 컬럼을 자동으로 선택한다 (기본값: None)
    ...
    if name == 'drop':
        ...
        if verbose:
            print(f"결측치 처리 방식: 'drop' (대상컬럼 {len(columns)}개)")
...
def pca(df, y=None, n_components=0.8, scale=True, method='standard',
        ...
        palette='coolwarm', hue_palette='tab10', width=1280, height=640, random_state=RANDOM_STATE):
    ...
    Args:
        ...
        n_components (float|int|str) : 남길 주성분의 기준 (기본값: 0.8)
            ...
            - 정수 : 주성분의 개수
            - 'mle' : 최대우도추정으로 개수를 자동결정
        scale (bool) : 분석 전에 스케일링을 수행할지 여부 (기본값: True)
        method (str) : scale=True 일 때 사용할 스케일러 이름 (기본값: 'standard')
        save_path (str, optional) : 학습된 PCA 객체를 저장할 pkl 파일경로 (기본값: None, 저장하지 않음)
        report (bool) : 주성분 로딩 백터표를 출력할지 여부 (기본값: True)
        ...
        random_state (int) : 재현성을 위한 랜덤시트 (기본값 : RANDOM_STATE)
        
    Returns:
        DataFrame : 주성분 점수(PC1, PC2, ···) 데이터프레임. y를 지정했다면 종속변수가 함께 붙는다
    ...
    # 2) 스케일링
    # 단위가 큰 변수가 분산도 크게 잡혀 주성분을 독점하므로 눈금을 통일한다 (PCA에서는 사실상 필수)
    if scale:
        work = scaling(work, method=method, verbose=False)
    
    # 3) 주성분 분석 수행
    # 3-1) 전체 주성분의 누적 설명력 확보
    # 몇 개를 채택할지 판단하려면 잘라내기 전의 설명력 곡선이 필요하므로 먼저 전부 구해둔다
    cumulative = PCA(n_components=None, random_state=random_state).fit(work).explained_variance_ratio_.cumsum()

    # 3-2) 기준만큼만 남긴 주성분 추출
    # n_components에 준 기준(개수 또는 누적 설명력)까지의 주성분을 실제 결과로 사용한다
    estimator = PCA(n_components=n_components, random_state=random_state)
    score = estimator.fit_transform(work)
    
    # 4) 주성분 점수를 데이터프레임으로 구성
    # 4-1) 주성분 이름 생성
    # 주성분의 이름은 관례에 따라 PC1, PC2 ...PCn으로 붙인다
    ...
    if plot:
        ...
        # 누적 설명력 임계선 (n_components를 0~1 실수로 지정한 경우에만)
        ...
        # 로딩 히트맵 : 진한색(양수/음수 양쪽 끝)일수록 그 주성분을 대표하는 변수
        # 설명력 행 두개는 로딩이 아니므로 제외한다
    ...
    if plot and biplot is not None:
        ...
        if isinstance(biplot, str):
            if biplot.lower().strip() != 'all':
                raise ValueError(f"biplot에는 2차원 리스트 또는 'all'만 지정할 수 있습니다: '{biplot}'")
